/* 
 * servertest.c - tests the server module with integration testing
 *
...
gameState_t* initializeGameState(int gold, char* mapfile);
bool handleMessage(void* game, const addr_t from, const char* message);
void handleSpectator(gameState_t* gameState, char keystroke);
void updateWorld(gameState_t* gameState);
void gameOver(gameState_t* gameState);
void handleSpectator(gameState_t* gameState, char keystroke);
bool haveSpectator(gameState_t* gameState);
void removeSpectator(gameState_t* gameState);
void gameStateDelete(gameState_t* game);
bool initializeMessage();
void updatePlayerWorld(void* arg, const char* key, void* item);
void sendTable(void* arg, const char* key, void* item);
void makeTable(void* arg, const char* key, void* item);
void handlePlayMessage(gameState_t* gameState, const addr_t from, const char* content);
void handleSpectateMessage(gameState_t* gameState, const addr_t from);
void handleKeyMessage(gameState_t* gameState, const addr_t from, const char* content); 
void sendInitialGame(gameState_t* gameState, const addr_t from);
void sendGrid(gameState_t* gameState, const addr_t from);
void sendGold(gameState_t* gameState, const addr_t from, int collected, int purse);
void sendDisplay(map_t* map, const addr_t from, char* playerID);
void sendOK(player_t* player, const addr_t from);
void sendGameOverMessage(char* table, const addr_t to);
void runTests();
...
void runTests() {
    ...
    const char* testCases[][3] = {
        {"./server", "map.txt", NULL},           // valid case without seed
        {"./server", "../maps/main.txt", "123"}, // valid case with seed
        {"./server", NULL, NULL},                // invalid case, too few arguments
        {"./server", "nonexistent.txt", NULL},   // invalid case, unreadable map file
        {"./server", "map.txt", "-1"},           // invalid case, negative seed
        {"./server", "map.txt", "abc"}           // invalid case, non-integer seed
    };
    ...
    for (int i = 0; i < numTests; i++) {
        ...
        int argc = 0;
        while (argc < 3 && testCases[i][argc] != NULL) {
            argc++;
        }
        ...
        // Call parseArgs and catch any exit calls
        if (parseArgs(argc, argv) == 0) {
            printf("parseArgs passed.\n");
        }

        else {
            // if we returned non-zero
            printf("Test failed.\n");
        }
        printf("Testing making a gamestate\n");
        gameState_t* gameState = initializeGameState(GoldTotalSize, "../maps/main.txt");
        printf("Testing loading gold\n");
        loadGold(gameState->masterMap);
...
handleMessage(void* game, const addr_t from, const char* message)
{
    ...
    if (strncmp(message, "PLAY ", strlen("PLAY ")) == 0) {
        ...
        fprintf(stderr, "Player wanting to play: %s\n", message_stringAddr(from));
...
updateWorld(gameState_t* gameState)
{
    if (gameState->goldRemaining == 0) {
        ...
        char* m = convertMap(gameState->masterMap, NULL);
        printf("%s\n", m);
        free(m); // !!!
